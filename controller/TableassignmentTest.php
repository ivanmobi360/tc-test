...
class TableassignmentTest extends \DatabaseBaseTest{
  ...
  function testAssign(){
    ...
    $this->buyTickets('john', 'sony', $BASIC, 3); // 3 BASIC tickets 
    
    $bar = $this->createUser('mary');
    $this->buyTickets('mary', 'sony', $BASIC, 1); // 1 BASIC ticket
    
    $bar = $this->createUser('cuero');
    $this->buyTickets('cuero', 'sony', $PREMIUM_SEAT, 2); // - 2 PREMIUM seats
    
    $this->createUser('rolindo');
    $this->buyTickets('rolindo', 'sony', $PREMIUM_SEAT, 2); // - 2 PREMIUM seats
    
    $this->createUser('tiban');
    $this->buyTickets('tiban', 'sony', $PREMIUM_SEAT, 1); // - 1 PREMIUM SEAT
    
    $foo = $this->createUser('fabi'); //figuretti
    $buyer = new WebUser($this->db);
    $buyer->login($foo->username);
    $buyer->addToCart('sony', $PREMIUM_TABLE, 2);
    $buyer->addToCart('sony', $BASIC, 1); //1 basic ticket
    $buyer->addToCart('sony', $PREMIUM_SEAT, 1); //- 1 PREMIUM just to isolate somebody
    $this->completeTransaction($buyer->placeOrder());
    
    
    $this->createUser('obama');
    $this->buyTickets('obama', 'sony', $VIP, 1); //una mesa - A Full VIP table
    ...
    $buyer->addToCart('sony', $BASIC, 4); //1 basic ticket
  ...
  function createSonyData(){
    ...
    , 'copy_event' => 'aaa'
    , 'e_name' => 'SONY'
    , 'e_date_from' => date('Y-m-d', strtotime("+1 month"))  //'2012-05-15' //needs to happen in the future to be shown in merchant pages 
    , 'e_time_from' => ''
    , 'e_date_to' => ''
    , 'e_time_to' => ''
    ...
    , 'reminder_email' => ''
    , 'reminder_sms' => ''
    , 'c_id' => 2 //apparently this is validated
    , 'l_latitude' => '52.9399159'
    , 'l_longitude' => '-73.5491361'
    , 'l_id' => 2 //validated too
    , 'dialog_video_title' =>'' 
...
(7, 'VIP 1', 774.5, 421.5, 1, 3, 2, 9, 'sony');
  ...
  function okTableData(){
    return array(
      ...
      , 'counter' => '4'
      , 'table' => 't_0'
      , 't_0' => array(788, 789, 786) //array(10,15,11) //only this group will be validated
      , 't_1' => array(12,13,14)
  ...
  function okSeatData(){
    return array(
      ...
      , 'tickets' =>array(781, 782)//array(5,6)//only this group will be validated
  ...
  function okDeleteData(){
    return array(
      	'event_id' => 'sony'
      , 'counter' => '4'
      , 'tickets' => array(786, 788, 789)  //array(10,15,11) 
    );
  }
