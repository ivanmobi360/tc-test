...
class EventSummaryTest extends DatabaseBaseTest{
  ...
  public function testCreate(){
    ...
    $evt = $this->createEvent('First', $user_id, 1, '2012-01-01', '9:00', '2012-01-05', '18:00' );
    $this->setEventId($evt, 'aaa');
    $catA = $this->createCategory('CatA', $evt->id, 10.00);
    $catB = $this->createCategory('CatB', $evt->id, 4.00);
    
    $user_id = $this->addUser('seller2');
    $evt = $this->createEvent('Second', $user_id, 1, '2012-02-01', '9:00', '2012-02-05', '18:00' );
    ...
    $this->buyTickets($bar->id, 'cuatro', $cat4->id, 2);
    
    $baz = $this->createUser('baz');
    $this->buyTickets($baz->id, 'cuatro', $cat41->id, 2);
    
    //multiple categories
    $client = new WebUser($this->db);
    $client->login($foo->username);
    $client->addToCart('aaa', $catA->id, 1);
    $client->addToCart('aaa', $catB->id, 1);
