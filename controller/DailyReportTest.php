...
class DailyReportTest extends DatabaseBaseTest{
  ...
  public function testCreate(){
    ...
    $client->addToCart('first', $catA->id, 10);
    $this->completeTransaction($client->placeOrder(false, '2012-02-14' ), '2012-02-14');
    //$client->placeOrder(false, '2012-02-14' ); //pending
    //$this->db->commit(); return;
    
    $client->addToCart('first', $catB->id, 5);
    $this->completeTransaction($client->placeOrder(false, '2012-02-20' ), '2012-02-20');
    ...
    $client->addToCart('second', $cat2->id, 3);
    $this->completeTransaction($client->placeOrder(false, '2012-03-15' ), '2012-03-15');
    //$client->placeOrder(false, '2012-03-15' ); //pending
    
    $silas = $this->createUser('silas');
    $client = new WebUser($this->db);
    $client->login($silas->username);
    $client->addToCart('third', $cat3->id, 4);
    $this->completeTransaction($client->placeOrder(false, '2012-03-17' ), '2012-03-17');
    
    $volk = $this->createUser('volk');
    $client = new WebUser($this->db);
    $client->login($volk->username);
    $client->addToCart('first', $catB->id, 1);
    $this->completeTransaction($client->placeOrder(false, '2012-03-19' ), '2012-03-19');
    
    $magnus = $this->createUser('magnus');
    $client = new WebUser($this->db);
    $client->login($magnus->username);
    $client->addToCart('first', $catA->id, 3);
    $this->completeTransaction($client->placeOrder(false, '2012-04-01' ), '2012-04-01');
  ...
  /**
   * For tixpro we're going to need more concrete cases, like purchases made by outlet, and tour purchases
   */
  function testTC(){
    //For a fixture just run OutletTest::testCC()
  }
