...
class CustomerDetailsTest extends \DatabaseBaseTest{
  ...
  public function testCreate(){
    ...
    $evt2 = $this->createEvent('Pies Descalzos' , $shakira->id, 1, '2012-01-01', '9:00', '2014-01-10', '18:00' );
    $general = $this->createCategory('General', $evt2->id, 25.00);
    ...
    $client->addToCart($evt->id, $yellow->id, 3);
    ...
    $client = new WebUser($this->db);
    $client->login($bar->username);
    $client->addToCart($evt2->id, $general->id, 4);
    $this->completeTransaction($client->placeOrder());
    ...
    $client->addToCart($evt->id, $cat->id, 1);
    $txn_id = $client->placeOrder(); //incomplete
    //$this->completeTransaction($txn_id);
