...
class CurrentEventOverviewTest extends \DatabaseBaseTest{
  ...
  public function testTickets(){
    ...
    Utils::clearLog();
    $client->addToCart('aaa', $cat->id, 25);
    $txnid = $client->payByCashBtn();
    
    $this->buyTickets('cammy', 'aaa', $cat->id, 25 );
    ...
    $client->addToCart($id, $silla->id, 3);
    ...
    $client->addToCart($id, $cat->id, 7);
    //$client->placeOrder();
    $this->completeTransaction($client->placeOrder());
    
    $this->db->commit();
    
    $this->addTourListingTest();
    
  }
  
  function addTourListingTest(){
    //let's try to show a tour in the 'current' list
    $this->createVenue('Pool');
    $outlet = $this->createOutlet('outlet1', '0001');
    
    $bits = '001';
    //move this to a tour builder test
    $build = new TourBuilder($this, $this->createUser('seller5'));
    $build->event_id = 't0ur5';
    $build->outlets = $bits;
    $build->build();
