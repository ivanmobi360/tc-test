...
class MerchantInvoiceListTest extends \DatabaseBaseTest{
  ...
  function fixture(){
    ...
    $txn_id = $this->buyTickets($buyer->id, 'aaa', $cat1->id);
    
    $buyer = $this->createUser('bar');
    $this->buyTickets($buyer->id, 'aaa', $cat1->id, 3);
    ...
    $this->buyTickets('paris', 'bbb', $cat1->id);
    ...
    $this->buyTickets('quo', 'ccc', $cat1->id, 3);
    $buyer = $this->createUser('vadis');
    $this->buyTickets('vadis', 'ccc', $cat2->id, 2);
